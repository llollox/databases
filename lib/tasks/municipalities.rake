namespace :municipalities do
  ...
  task :set_custom_db_config_paths do
    ...
    ENV['SCHEMA'] = 'db/db_municipalities/schema.rb'
    Rails.application.config.paths['db'] = ['db/db_municipalities']
    Rails.application.config.paths['db/migrate'] = ['db/db_municipalities/migrate']
    Rails.application.config.paths['db/seeds'] = ['db/db_municipalities/seeds.rb']
    Rails.application.config.paths['config/database'] = ['config/databases/municipalities.yml']
  end
  ...
  namespace :db do
    ...
    task :restore => :set_custom_db_config_paths do
      Rake::Task["db:drop"].invoke
      Rake::Task["db:create"].invoke
      exec "pg_restore --host localhost --username postgres --clean --no-owner --no-acl --dbname municipalities_development #{Rails.root}/db/db_municipalities/dump.dump"
    end
